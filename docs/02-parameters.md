...
| `patch_seconds` | 0 | **头部补丁长度（秒）**。0 = 关（逐位直通）。要开先试 **0.2–1.2**：补丁区 = 上一段床声 + 电平对齐的渐入，盖住段首 priming 静默即可；过长会把本段自己的起拍/内容吃掉（实测 1.2s 曾把 BGM 起拍压平）。🛡 **0.6.5 起默认有台词守卫**：本段头部台词起点被自动探测，patch 收缩到台词前 0.25s（`patch_guard=0` 关闭）。⚠ 必须先在**第 1 段**就接本节点（第 2 段的床源是它落盘的音频） |
