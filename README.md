...
### 🥇 最便宜的一刀：采样链（2026-09-19 实测）

> 这一条**不是本包的功能**（本包不含采样器，输出是张量），而是实测出来的一条捷径。

同 seed、同提示词、同参考图、同参数，**唯一变量 = 采样链**；672×1184（0.796 MP）/ 90 帧 / 2 段 / cond 桥：

| | 经 **SelfLift**（低分辨率相 + 学习型 latent 放大 + Refiner） | **官方链**（`SamplerCustomAdvanced` + euler，全程高分辨率） |
|---|---|---|
...
- **画面**上官方采样链明显更好：**"糊"的大头是链路的整段细节损失，不是段头那几帧的"settle 糊区"**
  —— 后者在 0.8 MP 下两条链都只剩 −2% / −15%（此前把它当主症状，是被低分辨率快测档放大了）。
- **代价**：官方链的**音频缝更差**（0.332 vs 0.909）。若两者都要，音频走本包音频缝节点的 **`joined`** 输出
  或组装层配方（见「续接音频缝」节）。
- ⚠️ **三条边界**：① 这是**链的整体对照**（采样器 + 有无低分辨率相 + 有无 Refiner + 有无 CFG
  四项打包），**不能**归因到 `euler` 单个参数；② 在 **cond 桥 / 2 段 / 0.8 MP** 上测的，copy 桥**未测**；
  ③ 官方链的 `BasicGuider` 没有 `negative` 槽 ⇒ **CFG 恒为 1**。
- ⇒ 只想改一样东西换画质，**先改这个**，它比调 Post 的旋钮有效得多。
