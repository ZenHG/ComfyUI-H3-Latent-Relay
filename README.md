...
| `settle_sharpen` | —（首段不裁） | 保持 `0`（需要时再开） | **裁重叠**节点 |
...
> "22 个 widget 的节点没法用"。再把不同域的旋钮合回一个节点（Post 15 个 + 音频缝 4 个 = 19 个）
...
>
> 走脚本产线（`l1_api.py`）不受影响——图是每次现构造的，永远取当前节点定义。
...
时**一帧沉降都不裁**——实测「裁沉降」才是缝处跳帧的源头（裁 0 帧跳 0.020「几乎无感」／
裁 8 帧 0.044「跳了」／裁 16 帧 0.055「跳」），留下的只是**清晰度的渐变**，人眼容忍度极高。
...
裁重叠会吃掉续接段头部 = **`trim_frames`（钉住区，默认 22 帧）+ `settle_frames`（沉降，自动检测上限 36 帧）**，
默认参数下最坏 ≈ **58 帧 ≈ 2.42s**（实测典型值：cond 桥 ≈ 38 帧 ≈ 1.58s）。对话排在这里会被**拦腰裁掉**（实测教训：台词起音被裁）。

⇒ 续接段开头 **≥ 裁头 + 0.2s 不排台词**（默认参数下 ≥2.6s）；续接段的 `[Shot 1]` 只写画面与机位承接，
