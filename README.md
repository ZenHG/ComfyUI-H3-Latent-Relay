...
> "22 个 widget 的节点没法用"。再把不同域的旋钮合回一个节点（Post 16 个 + 音频缝 4 个 = 20 个）
...
| `guide` | — | 接裁重叠的 `prev_tail`（= 上段末帧）。**只有跨段两项需要它**；不接时那两项自动跳过 |
...
| `hist_match` / `wb_match` | 0 / 0 | **段内**对齐（基准 = 本段段体）：直方图分布 / 灰世界白平衡 |
| `deconv_strength` / `_radius` | 0 / 1.5 | 反卷积去模糊（Wiener），提升段头高频 |
| `detail_borrow` / `_blur` | 0 / 9 | 段体高频迁移（把段头高频换成段体的结构） |
| `settle_sharpen` / `_frames` | 0 / 24 | 糊区锐化（unsharp，渐变）。**不裁、不动时间轴 ⇒ 不可能引入跳帧** |
