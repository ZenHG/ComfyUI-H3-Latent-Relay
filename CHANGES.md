...
> **工具版**：把「音画不同步」的**接线错误**做成机检 —— 这是 UI 侧唯一「节点自己发现不了、但机器能抓住」的东西。无运行时改动，默认行为与 0.6.2 逐位一致。

### 🔎 `check_ui_workflow.py` 新增「A/V 同步接线」判据

**为什么必须由工具查、节点查不了**：`H3RelayTrimAV` 只能发现「我的 `audio` **输入**没接」（会打警告），**发现不了「我的 `audio` 输出被悬空」** —— ComfyUI 不会告诉节点「我的输出有没有被消费」。于是最常见的错法是 `VAEDecodeAudio → CreateVideo.audio`（= **未裁**的原始音频），而「裁重叠 `[1] audio`」那根线悬空 ⇒ 画面裁了、音频没裁⇒ **每缝差 ~0.9s、逐段累积**（第 3 段起口型明显对不上）。

**判据**：图里存在续接桥（`H3RelayCopyBridge`，stage≥1 时 `trim_frames` 非 0）时，落盘节点的 `audio` 上游若为 `VAEDecodeAudio` / LoadAudio 家族 ⇒ **判 problem**；接了「裁重叠 `[1]`」或「音频缝 `[0]`」⇒ 通过。没有桥（单段图）时两者都不裁 ⇒ 不报（不误伤）。

**验证**：两个对照样本（坏 = 原始音频直连 / 好 = 走裁重叠 `[1]`）—— 坏样本被抓、好样本 0 误报；`examples/minimal_relay_official.json` 复检 **问题合计 0 条**（示例图本身接线是正确的：`CreateVideo.audio ← H3RelayAudioSeam[0]`）。

## 0.6.2 — 2026-09-21

> **补丁版**：把「音画同步」这条线**在节点层彻底收口**（不再靠组装层兜），
> 并把「**UI 与 API 必须同一套实现、且必须基于节点**」立为铁律。
> 门槛：`test_relay_core` **286/0**（+第 23 组）· `test_experimental` 57/0 · `review_050` **78/0**（+L10）· `smoke_nodes` 15/0。

### 🔴 修：`joined` 的默认档会缩短时间轴 —— 每缝音频提前 50 ms、逐段累积

- **现象**：`音频缝` 第 3 路输出 `joined` 的默认档是 `join_cross_ms=50` + `join_align_seconds=0`
  ⇒ 走「旧缩短语义」。交叉淡变是 **overlap-add** ⇒ 每缝使后段音频相对画面**提前 50 ms**，
  且**逐段累积**（第 3 段起口型明显对不上）。而它只在一行 report 里带个 ⚠
  ⇒ 属于「**静默产出一条错位音轨**」。
- **处置（三层）**：

  | 层 | 改法 |
  |---|---|
  | **默认不再缩短** | `join_cross_ms` 默认 **50 → 0** ⇒ 默认 = **等长拼接**（与「段文件首尾相接」等价，**零 A/V 位移**） |
  | **错配置直接 raise** | `join_cross_ms > 0` 而 `join_align_seconds = 0` ⇒ **报错**，并给出两条出路（① 填 `join_align_seconds` 走 J-cut 守恒路；② `join_cross_ms` 填 0 走等长拼接）。不留「忽略参数」的中间态 |
  | **报告不再假警告** | `cross=0` 时不再写「⚠ 缝后音画错位」（那时根本不存在缩短），改称「**等长拼接（不做交叉；等效于段文件首尾相接，零 A/V 位移）**」 |

- **顺手把正确值算给用户**：`裁重叠` 的 report 新增一行 ——
  「▶ 音频拼接（AudioSeam 的 joined）：J-cut 守恒路的 `join_align_seconds` 填 **0.9167**
  （= 裁首 22 帧 ÷ 24 fps）」。以前要用户自己换算 fps，于是没人填对、默认就落在缩短语义上。

### 🔴 铁律：UI 与 API 必须同一套实现，且**必须基于节点**

本包**多数用户是用画布手动跑的**（改段号 → 点 Queue → 改 prompt → 再点 Queue），
只有少数走 API 提交图 JSON。所以「只在脚本里生效」的改动，等于对多数用户**不存在**。

- `CONTRIBUTING.md` 立为**铁律一 / 铁律二**（含 PR 自查三条 + 真实反例）；
- `README.md` 开头「本文约定」加第 ④ 条；
- `tools/review_050.py` **L10** 机检这条规则还在不在 ——
  规则若只活在人的记忆里，下一个人照样会去改私有脚本。

### 🧪 测试

- `tests/test_relay_core.py` 新增**第 23 组「音画同步守恒」5 项** ⇒ **286/0**（原 281），方面数 22 → **23**；
  `review_050` **78/0**（新增 L10）。README / CONTRIBUTING / `docs/08` / `tools/README` / ISSUE_TEMPLATE 的计数已同步。
- ⚠️ 23.2 断言的是「**零时间轴位移**」（后段只被整段增益缩放、样本位置一一对齐），
  **不能**断言「逐位相同」—— 后段会被**跨段响度匹配**整体缩一个标量（±6 dB 限幅）。
  23.3 走**节点完整调用路径**验守卫可达；23.5 验 TrimAV 报告里的建议值。

## 0.6.1 — 2026-09-21

> **补丁版**：修两个会让成片听感/同步出错的问题，并把两个**只读观测量**接进产线路径。
> **默认行为不变** —— `test_relay_core` 281/0、`test_experimental` 57/0、`review_050` 77/0、`smoke_nodes` 15/0。

### 🔴 修：音频缝的床窗会撞上台词，把上一段台词整句搬进本段头部

**现象**：`patch_seconds > 0` 时，补丁取床源的「尾部窗」整段替换本段头部。
若**床源尾部含台词**，整句台词会被复制到下一段头部，再叠上边界交叉淡变
⇒ 成片听感是「**对白重叠**」（实测：段 1 台词覆盖 3.0–4.0s，补丁窗完整包住它）。
⚠️ **默认档（`bed_select=tail`）同样会撞** —— 不是开了实验档才有。

**修法**：新增 `bed_window_voiced_fraction` —— 帧 RMS 超**全源中位 3×** 记为有声帧，
窗内有声占比 > `AUDIO_SEAM_BED_VOICED_FRAC`（10%）判为「撞语音」。

| 分支 | 撞语音时改取 |
|---|---|
| `tail`（默认） | **非语音窗里能量最高**的那个 —— 避开语音的同时避开静音洞 |
| `exp_bed_jitter` 指定窗 | **非语音候选里离指定点最近**的 —— 保留「按段号错开」语义 |
| 全部候选都撞语音 | 退回 `quietest_window`（旧口径） |

日志新增 `🎙 窗内有声帧占比 X%`；撞语音并换窗时显式注明。

### 🔴 修：`audio_seam_patch` 在 `patch_seconds > 0` 时抛 `NameError`

报告行引用了 `build_bed` 的**形参名** `start_override`，而本函数里叫 `_ov`
⇒ 节点抛错 ⇒ **整条链静默失败**（采样跑完了却不出 mp4）。
已补 `E5.9–E5.11` 三条断言覆盖**节点真正调用的完整路径**（此前只测 `build_bed` 内部，所以漏掉了）。

### 🧪 两个只读观测量接进产线路径（原先都不可达）

| 量 | 原先 | 现在 |
|---|---|---|
| **E3** `DTW 对齐代价` | 挂在「自动裁沉降」分支内 ⇒ 产线 `settle_frames=0` 时**永不执行** | 移出分支 ⇒ 每段固定打印（只读观测，**不进裁量契约**） |
| **E4** `外观三元组 / 漂移曲线` | `nodes.py` **零引用** | `H3RelayTrimAV` 新增可选输入 **`run_id`**（**末位追加**）⇒ 每段把外观三元组追加进 `<run_id>/appearance_log.jsonl`，并读历史算漂移斜率打进 report |

不填 `run_id` ⇒ E4 静默 no-op，行为与旧版一致。观测量参考区间见 [`docs/09-metrics.md`](docs/09-metrics.md)。

### 📖 文档（这批才是多数用户真正会踩的）

- **README §4 接线图补上 `[1] audio → CreateVideo.audio`** —— 原图只画了音频**输入**侧。
  用户照图最自然的手会把 `VAEDecodeAudio` 直连落盘节点（= 未裁的原始音频）
  ⇒ **画面裁了、音频没裁** ⇒ **音画不同步且逐段累积**（第 3 段起口型明显对不上）。
  节点只能发现「输入没接」，**发现不了「输出被悬空」**，所以这条只能靠文档盯住。
- **README 新增 §7.1「多段拼接」**：段文件「**音频长度 = 视频长度、段间无重叠**」契约；
  两种错误拼法的症状（`-f concat` demuxer → 每缝 ~0.87s 静止画；`acrossfade` → 每缝偷 0.25s）；
  照抄可用的命令 + 拼完自检。
- **README §5** 明确 `H3RelayChain` **本身不拼片**（只把 N 段跑完，N 个 mp4 的拼接见 §7.1）。
- **README §9 排障**补三条：音画越到后面越不同步 / **连跑出来的每段台词一模一样**（连跑是反复提交同一张图，词不换）/ 拼完每缝有静止画。
- 新增 [`docs/09-metrics.md`](docs/09-metrics.md)：观测量参考区间与自校准方法。

### 🧪 实验层（`exp_*` 控件，**默认全关 = 主干逐位不变**）

- `H3RelayCopyBridge` 追加 `exp_history_depth` / `exp_history_stride` / `exp_cond_noise`；
  `H3RelayAudioSeam` 追加 `exp_bed_jitter`（**一律末位追加**，旧工作流取值不前移）。
- 新增 `tests/test_experimental.py`（E1–E5 的零 GPU 断言，**57 项**）。
- 状态如实说明：**E3/E4 是只读观测**（已接线）；**E1/E2 在研**（E1 的「撞全局锚源」与
  `stage_index<2` 静默档待修，E2 与「硬锁」语义互斥）；**E5 在本案素材上无可听收益**
  （`jitter` 0 vs 1.2 在补丁区只差 0.1–1.3 dB），保留待长链素材复测。
...
- **`mask_mode` tooltip 撤下一条作废结论**：旧写「ramp 与 hard 三项完全等同（0.0407 vs 0.0402）⇒ 无增益」——
  ...
  的判据重测：**四个掩码档在缝处全都跳**，只是失败模式不同。
  （判据与原始记录在作者本机的 `_handover` 调研笔记里，不随包分发。）
