...
> **（同日补 9）修 V3 机检的「节点数 / input 数」错误前提 + 计数自洽 + 这组数字终于可机检** ——
> GG 指出「我们实际就是 8 个节点啊」，顺着查出一条**从 v0.6.8 起就在的错误前提**。
>
> 🔴 **错误前提**：`ci.yml` / `docs/08` 一直写着「CI 里是 7 节点 / 98 input，本机才是 8 节点 /
> 111 input」，理由 =「CI 没装上游超分节点包 ⇒ `H3RelayLatentUpscale` 诚实降级被跳过」。
> **实测不成立**：`latent_upscale_models` 是**宿主 ComfyUI 自带注册**的目录
> （`folder_paths.py:43`），不是上游超分包装的 ⇒ `_upscale_model_names()` 走
> `get_filename_list()` 正常返回（最多给占位串）、**永不抛错** ⇒ `INPUT_TYPES()` 不抛
> ⇒ V3 entrypoint 的逐节点容错**根本不会触发** ⇒ **节点数恒为 8，与环境无关**。
> 验证手法：把上游节点从宿主注册表里摘掉，`get_node_list()` **仍是 8 个**。
> ⇒ CI 与本机**恒等**：**8 节点 / 112 个 input**。那组偏小的数字只活在**注释**里、无人断言
> ⇒ 所以 CI 一直绿着，而文档一直是错的。
>
> **① `test_v3_schema.py` 两处判据修正**
> · `2.1` 原判据 `len(V3_NODES) == (8 if _HOST_REGISTRY_OK else 7)` —— 前提错，改为**恒等于 8**；
>   `2.2` 同步收紧（不再允许 Upscale 缺席）。
> · `_N_INPUT` 原来只统计「V3 里**匹配到**的 input」⇒ 某个 input 缺席时**总数静默变小**
>   （结果行照样打印"逐项比对的 input N 个"，看不出是"缺失"还是"本来就这么少"）。
>   新增 `_N_DECL` 统计「**V1 声明**的 input」，并在新增的 **[7] 计数自洽**节断言两者相等。
>
> **② 新增机检 H3h**（`tools/review_050.py`）：核 `ci.yml` · `docs/08` · `README` · `__init__`
> 四处的 **节点数 / input 数 / 机检项数**与实跑一致。**过去加不了**（两边数字不同，一处表达不了
> 两种环境，只能人工维持）⇒ **现在两边恒等，终于可机检**。正则刻意收窄：节点数只认
> "与 input 同行"的写法（README 里"3 节点 / 18 节点 / 45 节点"是别的意思），
> 且更正说明里**不许复述旧数字**（历史只留在 CHANGES 与本地审核文档里）。
>
> **③ 顺带**：H3f 的正则早已改成"逐行扫、双向认"，`ci.yml` 里 review_050 的期望数由 **H3g**
> 自引用盯着（本次它当场报红 84→85）。
>
> **门槛**：test_relay_core **392/0** · test_v3_schema **69/0（8 节点 / 112 个 input）** ·
> review_050 **85/0** · smoke 15/0 · dispatch 29/0 · 默认出口 **4/4** · 一键套件 **7/7** · ruff 通过。
...
>
> **门槛**：test_relay_core **392/0** · test_v3_schema **69/0** · review_050 **86/0** ·
