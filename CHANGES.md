...
> **（同日补）实验层 E1/E2/E5 移除，E3/E4 转正为只读观测** —— 三个干预类实验全部实测无效：
> **E1 多尺度历史**（单变量 A/B，同 seed 同 latent，唯一变量 depth 0→2）：refs 通道验证**真生效**
> （生成区像素差 37/255），但跨段漂移**不减反增**（|s5−s3| 亮度差 ×2.9、锐度差 ×9，段5 亮度掉 15.7）；
> 设计层根因：refs 取历史段头部 = 横移镜头下的**陈旧机位**，运动镜头下没有一档内容是对的。
> **E2** 与硬锁互斥（把参考弄脏）；**E5** 三缝差 0.1–1.3 dB 无可听收益。
> E3（DTW 残留）/E4（漂移曲线）为**只读观测**，保留并作为日后画质域治理的验收工具。
> 连带清理：nodes.py/relay_core.py 的 E1/E2/E5 实现与 widget、tests（experimental 60→29 条）、
...
> **（同日补 3）修正「分块不换画面」的错误声明（3 处，纯文案）** —— 外部复核发现节点自述与实测结论
> **互相矛盾**：`H3RelayLatentUpscale` 的类 docstring 写「⇒ **换块数不换画面**」、`chunks` tooltip 写
> 「画面上**与 1 块等价**（有单测锁这条）」、节点 `DESCRIPTION` 写「**分块与整段结果等价**」；
> 而 README §9 排障表与 `docs/02-parameters.md` 的实测结论恰恰相反：**`chunks>1` 会改画面**
> （同 seed 同参 `chunks=1` vs `4`：逐帧 MAE **5.62/255**、192 帧里 0 帧逐位相同），
> 根因是上游内部 `AttnBlock3D` **体积注意力**——切块 = 换跨块时间上下文，`overlap` 渐变只能缓解不能抵消。
> 第 27 组单测锁的是**拼接数学**（逐帧独立算子下 `分块 == 整段`，max|Δ| = 5.96e-08），**不是「画面不变」**；
> 两者被混为一谈 ⇒ 用户照 tooltip 调大 `chunks` 降显存会**静默改变画面**而不自知。
> 修正 = 三处一律改为「单测锁的是拼接数学，不是画面；`chunks>1` 会改画面，升完必须重看缝」，
> 并指向 README §9 / docs/02 的实测数据。**算法零改动**；门槛：test_relay_core **384/0**（本机）。

> **（同日补 4）修 `hist_match` 在高分辨率下必崩 —— 真 bug，不是性能问题** —— 外部复核给的第二批
> 20 条**性能**建议里，唯一真正该动手的这条恰恰**不是**性能问题：`match_hist_head_to_body` 对
> **整帧像素 flatten** 求分位，而 `torch.quantile` 有 **numel 硬上限 2^24 = 16,777,216**，
> 超过即抛 `RuntimeError: quantile() input tensor is too large`。
> 实测各画幅「每通道可容帧数」：**0.31MP → 54.6 帧 · 0.80MP(672×1184) → 21.1 帧 · 1.03MP(768×1344) → 16.3 帧**；
> 而 90 帧段的段体有 50 帧（`robust_body` 筛后 ~25 帧 = **19.9M**）⇒ **0.80MP 起必然超限**，
> 即 Post 的 `hist_match` 开关**一开就崩**。⚠ 0.3MP 时代测不出来（那是**分辨率红利**，不是设计）——
> 外部建议书给的是「2.93 s → 几十 ms」的性能口径，正因为它**在 0.3MP 下测的**（那里每通道容 54.6 帧）。
> 修法 = `_quantile_capped`：未超限**逐位等价**原实现（0.3MP 及以下行为不变）；超限时等间隔子采样。
> 🔴 **采样目标取 2^21 而不是「刚好 2^24」是关键**：实测 2^24 点 quantile 要 **3.5 s**，
> 而本函数是**逐通道 ×(段体+段头) = 6 次** ⇒ 2.0MP 段体 51.8M 点采到 13M ⇒ **单段 18 s**
> （不崩，但等于把节点卡住）；采到 2^21 ⇒ **单段 ~2.3 s 且与分辨率无关**（2MP/4MP/8MP 同级）。
> 精度代价：2^21 点分位标准误 3.5e-4 ≈ **0.09/255**，仍比 256 桶的桶宽（3.9e-3）小一个量级 ⇒ 肉眼不可见。
> 门槛：test_relay_core **389/0**（+5：12.44 未超限逐位一致 · 12.45 超限不抛 · 12.46 精度 ·
> **12.47 采样点数与输入规模脱钩** · 12.48 端到端；⚠️ 12.47/12.48 两条的编号在「同日补 8」里对调过一次
> ⇒ 现在文件内按出现顺序递增）。
> 连带文档：README **§9 排障表**新增「开了 `lowfreq_pull` 后每段等 20 多秒」（含 k 曲线实测与
> 降档建议：2.07MP 下 k=64→22.1 s · k=32→5.6 s · k=16→1.6 s）、§10.2 加指针；
> `docs/02` 补 `lowfreq_pull._blur` 的「耗时 ∝ k²」与 `hist_match` 的子采样说明。

> **（同日补 5）TrimAV 三路只读观测改为 opt-in（`diagnostics`，默认关）** —— GG 拍板：
> **对外默认关、本地默认开**。三路观测（🧪 DTW 代价 E3 / 裁量→跳跃曲线 / 🧪 外观三元组+漂移曲线 E4）
> **都不参与任何裁量**（纯打印）⇒ 关掉**不改变任何帧/latent/音频**，只省 CPU（实测三路合计
> **0.361 s/段** @0.796MP/90 帧；2.0MP 约 0.5–1.5 s）。新增 optional **末位** widget `diagnostics`
> （BOOLEAN，默认 `False`，追加在 `save_pcm` 之后 ⇒ 守 19.7 的槽位契约）。
> ⚠️ **本地产线要常看这三行** ⇒ 跑批入口**显式传 `True`**（`l1_api.py` 的 903 节点构造处；
> `chain_auto.sh` 经它转发）—— 不依赖节点默认值，守「参数静默回退」纪律。
> ⚠️ `settle_frames=-1` 分支里的 `detect_settle` / `boundary_jump_ratio` / `observation_profile`
> 属**裁量链路**，**不受**本开关影响。
> 门槛：test_relay_core **392/0**（19.7 尾部延长一位 · 新增 **19.7b/c/d** 三条：默认关无三路观测 /
> 开则全回 / **开关不改变裁切结果（逐位相同）**；⚠️ 这三条原先编号 19.8，与既有的「19.8 默认全关」
> **撞号**，在「同日补 8」里改为本文件既有的 `b/c/d` 子编号惯例）。

> **（同日补 6）修 `/h3relay/concat` 阻塞事件循环（拼接期间 UI 全冻结）** —— 外部第三批建议里
> **唯一指向真问题**的一条（虽然它给的解法治不了）。路由是 `async def`（跑在 aiohttp 事件循环里），
> 却在里面**同步调用** `assemble_mp4_segments`（ffmpeg 编码，几十秒级）⇒ 事件循环被整个堵住，
> 期间 UI 的**所有**请求（队列轮询 / `/queue` / `/history`）全部无响应 ⇒ 表现为「点了没反应 / 卡死」。
> 实测对照（1.5 s 重活，`I:/_seam_tmp/probe_aiohttp_block.py`）：
> **同步调用下 `sleep(0.15)` 实耗 1.502 s（循环被占满）；`run_in_executor` 下 0.155 s 照常返回。**
> 修法 = `await asyncio.get_running_loop().run_in_executor(None, lambda: …)`。
> ⚠ 建议书原方案 `asyncio.wait_for` **治不了** —— 超时能抛，但同步调用仍占着循环。
> 全仓**仅此一个路由**（已核），无其它阻塞点。
> 门槛：test_relay_core **392/0**（路由标 `pragma: no cover` ⇒ 由语法编译 + 上述离线对照实验守）。
>
> ⚠️ **连带补一个「改动自己引入的风险」**：丢线程池后，两个并发拼接请求会**同时**进
> `assemble_mp4_segments` ⇒ ① 都走 `while os.path.exists(out_path)` 可能**撞同一个输出名**、同时写
> 同一文件；② PyAV/ffmpeg 并发编码。**改之前它们在事件循环里被同步调用 ⇒ 天然串行**，这个隐含保证
> 丢了 ⇒ 显式加 `asyncio.Lock`（`_CONCAT_LOCK`）把「算输出名 + 体检 + 拼接」整块串行化补回。
> 🔴 **必须是 `asyncio.Lock` 而非 `threading.Lock`**：后者的 `with` 是**同步**的，等锁时会**再次堵住
> 事件循环**，等于白改。
> 实测（`I:/_seam_tmp/probe_concat_serial.py`，0.8 s 重活 × 3 并发）：
> 无锁 **0.807 s**（并行 ⇒ 会撞名）｜有锁 **2.404 s**（串行 ✓）｜两者期间 `/ping` 均 **0.06 s**（都不堵循环）。

> **（同日补 7）加「环境自检」两件（7.4）** —— 外部第三批建议里唯一值得吸收的**新增功能**。
> 动机对着 `.github/ISSUE_TEMPLATE/bug_report.md` 算账：它要用户填「本包版本 / ComfyUI 版本 /
> Python+torch / OS」，全靠手动翻；本包特有的三项（节点注册数 / 时序契约 / 上游可选依赖）
> 更是没人会主动查 ⇒ 报一个 bug 要来回 **3–5 轮**问环境。
> **① 加载摘要（零门槛）**：包加载完打一行
> `[H3 Relay] v0.6.8 已加载｜节点 8 个（出口 v3）｜时序契约 ✓`
> —— 它同时是「包压根没加载」的**唯一判据**（那种情况连路由都不存在）：**这行没出现 = 没加载**。
> ⚠ 上游可选依赖**不在这里查**（包加载时宿主的 `nodes` 模块可能还没就绪，查了会误报"未装"）。
> **② `GET /h3relay/health`**：一次给全 issue 模板要的 + 本包那三项。⚠ 它同样**查不到"包没加载"**。
> 连带：issue 模板「环境」一节改成"先 curl 这个"；README §9 排障表加一行同样的两步走指引。
> 顺手修：`tests` 的 stub `_Routes` 原来**写死 `post`** ⇒ 加 GET 路由后 26.15 假红
> （`AttributeError: '_Routes' object has no attribute 'get'`）；改 `__getattr__` 兜底，
> 以后加任何 HTTP 方法都不用回来改它。
> 门槛：test_relay_core **392/0** · review_050 **82/0** · smoke 15/0 · dispatch 29/0 · 默认出口 ✓。

> **（同日补 8）H 仓「最新更新」审核：2 高危 / 3 中危 / 6 低危 —— 全部已修** ——
> 审核对象 = `1c87aff..bc395a4`（17 个提交，**已推**）＋ 当时**未提交**的 18 个文件。
> 结论：**「6 项闸门全绿」是假绿** —— 真问题全落在套件覆盖之外。
>
> 🔴 **高危 1：`tests/test_v3_schema.py` 导入即崩**（`AttributeError: '_Routes' object has no attribute 'get'`）。
> 当天新增 `GET /h3relay/health` 后离线桩 `_Routes` 必须支持 `get` —— 同类桩全仓 **4 处**，
> 当天只改了**报错的那一处**（`test_relay_core`），本文件漏了，而它**恰好不在一键校验套件里** ⇒ 假绿。
> `ci.yml` 会跑它 ⇒ **当时提交就是 CI 红**。修：`__getattr__` 兜底（以后加任何 HTTP 方法都不用回来改）。
> **教训**：契约性改动（加路由 / 加 widget）要**横向 grep 全部同类桩**，不能只修报错的那一个。
>
> 🔴 **高危 2：`__init__._node_names()` 的 V3 分支是死代码**。写的是 `from .v3 import nodes`，
> 而真实模块名是 **`v3/nodes_v3.py`** ⇒ 每次抛 `ModuleNotFoundError`、被 `except: pass` 吞掉
> ⇒ **从来没生效过**，一直静默退回 V1 映射。当时没暴露，是因为两套清单 1:1、退回去算出来
> **还是 8 个、名字也对** —— **结果正确 ≠ 代码正确**。真风险：一旦 V1/V3 分叉，banner 与
> `/h3relay/health` 会**静默报 V1 的清单，而宿主跑的是 V3**。修：走对模块名 ＋ `except` 分支
> **必须出声**（不静默降级）＋ 新增机检 `test_v3_schema` **6.1/6.2**（判据 = **返回 NODES 序而非字典序**
> —— 这正是当初一眼看穿"走了哪条分支"的那个差异）。
>
> 🟠 **中危 3：部署副本落后一整天**（H 仓 09-25 / I 盘 09-24）⇒ `l1_api.py` 传的 `"diagnostics": True`
> 被宿主**静默忽略**（`execution.py` 的 `valid_inputs` 只遍历节点**自己声明**的输入项，多余键不报错）
> ⇒ 「本地默认开」当时是**空操作**且无任何提示。修：同步副本 ＋ 重启。
> 🟠 **中危 4：一键套件漏 `test_v3_schema`**（`grep -c` = 0）—— 它恰是守护「默认出口切 V3」的那道机检，
> 唯一没收进去的就是它 ⇒ 这就是"6/6 全绿"没抓到高危 1 的直接原因。修：补进表 ＋ 裸 `node` 改显式解析。
> 🟠 **中危 5：`ci.yml` 自相矛盾**（头注释 83 / 执行行 82 / 真值 83）。**H3f 的正则要求数字在文件名之前**
> ⇒ 只匹配得到注释行、**执行行永远漏检**，且根本没查 `review_050` 自己的数。修：H3f 改**逐行扫、双向认**；
> 新增 **H3g 自引用**（`ci.yml` 里 review_050 的期望数 == 本文件实跑真值，**必须是该文件最后一个检查**）
> ⇒ 以后加检查忘了同步，它当场报红。
>
> 🟡 低危 6 条：死常量 `QUANTILE_MAX_ELEMS`（逻辑从不引用 ＋ print 把 2^21 门槛说成 2^24）·
> 测试编号 `19.8` **撞号 4 次** ＋ `12.47/12.48` 乱序 · `dist/` 陈旧 · 部署副本里 0 字节垃圾 `36` ·
> `bug_report.md` 又写死一个总数（第 4 处同数字、无人机检）· 一键脚本用裸 `node`。
>
> **连带修掉的数字漂移**：今天加 `diagnostics` widget ⇒ V3 机检的 input 数 **111 → 112**，
> 而 `65/0 / 111 input` 在 **4 个文件**里写死（`ci.yml` · `docs/08` · `README` · `__init__`）
> ⇒ 一并更新（详见「同日补 9」——那一轮把节点数/input 数的**错误前提**也一起修了）。

> **（同日补 9）修 V3 机检的「节点数 / input 数」前提 + 计数自洽 + 这组数字终于可机检** ——
> GG 指出「我们实际就是 8 个节点啊」，顺着查这条线。**⚠️ 本条目记的是最终定案**，
> 中途我判错过一次（见「补 11」）——**结论是：两个数都要写，因为有两种合法环境**。
>
> 🔴 **为什么是两个数**（2026-09-25 实测钉死）：差的那一个节点是 `H3RelayLatentUpscale`，
> 它的 `INPUT_TYPES()` 会走到 `_comfy_registry()`，而后者**第一步就 `import nodes`**。
> · 宿主 `nodes` **可导入**（本机）⇒ 上游节点不在注册表 ⇒ `_upscaler_cls()` 抛 **RuntimeError**
>   ⇒ `_upscaler_module()` 捕获它（**只** `except RuntimeError`）⇒ 退回 `get_filename_list`
>   （宿主自带注册 `latent_upscale_models`，`folder_paths.py:43`）⇒ 不抛 ⇒ **8 节点 / 112 input**。
> · 宿主 `nodes` **导不进来**（CI：只 clone 宿主、不装上游包）⇒ 抛的是 **ModuleNotFoundError**
>   （ImportError 子类）⇒ **异常穿透**（没被捕获）⇒ INPUT_TYPES 抛 ⇒ V3 entrypoint 的逐节点
>   容错跳过它 ⇒ **7 节点 / 99 input**。**这是设计内的降级，不是失败。**
> ⇒ 判据跟着环境走（`_HOST_REGISTRY_OK`），**两个数并排写才是正确形态**。
> ⚠️ **模拟 CI 的正确姿势**：`sys.modules["nodes"] = None` 后跑该测试（实测 `62/0 · 7 节点 · 99 input`）。
> **别**用"只把上游节点从注册表里摘掉"——那复现的是「装了宿主、没装上游包」，那种情况**仍是 8 节点**。
>
> **① `test_v3_schema.py` 判据修正**
> · `2.1` / `2.2` 恢复**条件判据**（`8 if _HOST_REGISTRY_OK else 7`），并在注释里写清上面那套机制。
> · 新增的 **[7] 计数自洽**节改成**与环境无关**的断言：`_N_TOTAL == len(V3_NODES) == len(V3_BY_ID)`。
> · `_N_INPUT` 原来只统计「V3 里**匹配到**的 input」⇒ 某个 input 缺席时**总数静默变小**
>   （结果行照样打印"逐项比对的 input N 个"，看不出是"缺失"还是"本来就这么少"）。
>   新增 `_N_DECL` 统计「**V1 声明**的 input」，断言两者相等。
>
> **② 新增机检 H3h**（`tools/review_050.py`）：核 `ci.yml` · `docs/08` · `README` · `__init__`
> 四处的**节点数 / input 数 / 机检项数**。**语义 = 「当前环境的真值 ∈ 文档声明集合」**，
> 并限制每种数**最多 2 种取值**（只有两种环境）⇒ 既允许"两个数并排写"，又能抓到
> "写了个哪个环境都不产出的数"。正则刻意收窄：节点数只认"与 input 同行"的写法
> （README 里"3 节点 / 18 节点 / 45 节点"是别的意思）。
>
> **③ 顺带**：H3f 的正则早已改成"逐行扫、双向认"，`ci.yml` 里 review_050 的期望数由 **H3g**
> 自引用盯着。
>
> **门槛**：test_relay_core **392/0** · test_v3_schema **本机 69/0（8 节点 / 112 个 input）·
> CI 62/0（7 节点 / 99 input）** · review_050 **86/0** · smoke 15/0 · dispatch 29/0 ·
> 默认出口 **4/4** · 一键套件 **7/7** · ruff 通过。

> **（同日补 10）远端仓库梳理：清死代码 / 清已移除功能的残留文档 / 补两处机检盲区** ——
> GG 要求「梳理远端哪些文件要同步、哪些旧的可以删，确认无 bug 后 push」。
> 结论：本地与远端**都跟踪 52 个文件**，差异是 **21 个"修改"、0 新增、0 删除**（结构本来就齐）。
> 但梳理过程查出**四类必须一起带走的问题**：
>
> **① 死代码**：`v3/__init__.py` 的 `V3_NODE_IDS` 与 `v3/nodes_v3.py` 的 `NODES` 是
> **同一份清单的两处声明**，且**全仓零引用**（`grep -rn V3_NODE_IDS` 只命中它自己的定义；
> `tests/test_v3_schema.py` 里那处其实是**同名局部变量**的 walrus，与它无关）⇒ 已删除，
> 并把"人读版清单"降为 docstring 注释，**程序读一律走 `nodes_v3.NODES`**。
> 顺带修掉那个同名 walrus（会误导读者以为在用模块常量）。
>
> **② README 在给用户文档化一个已不存在的参数**（**用户可见缺陷**）：音频缝参数表里
> `exp_bed_jitter` 仍写着「**0，默认关**」像是可设项，而该参数已在 `1c87aff` 随实验层 E1/E2/E5
> 一并删除 ⇒ 用户照它设会被宿主**静默忽略**（多余输入键不报错）—— 正是本包一直在抓的那类失效。
> 已改为「**该参数已移除**」并保留原结论存档（将来若开 BGM 可能重启）。
>
> **③ 已移除功能的残留文档**：`docs/06` 的"怎么设置"表仍列 `E1/E2/E5 | 全关（默认）`；
> `docs/09` 头部写"适用版本：`exp/seam-frontier` 分支（实验层 E1–E5）"，而该分支**已删除**；
> `CONTRIBUTING` 的 0.6.4 根修叙述里多处引用 `exp_bed_jitter`。均已加**明确的移除标注**，
> **历史结论一律保留**（E1 的后续想法仍在，见 `memory` 待办）。
>
> **④ 两处机检盲区（本次新补）**：`docs/08` 与 `tools/README.md` 里的期望数
> **长期没跟上**（`review_050` 停在旧值、`assert_default_exit` 停在 `3/3` 而真值 `4/4`），
> 因为这两份文件**从来没被任何机检扫过**。修法：
> · **H3i（新）**：跑 `assert_default_exit`，核它的 `N/N` 在 ci.yml · docs/08 · tools/README 一致；
> · **H3g（扩）**：从"只扫 ci.yml"扩到 **4 个文件**（+ docs/08 · tools/README · README）。
> 顺带按"更正说明不许复述旧数字"的纪律，清掉了 README 叙述段里的旧数字（否则会污染新机检正则）。
>
> **门槛**：test_relay_core **392/0** · test_v3_schema **本机 69/0 / CI 62/0** · review_050 **86/0** ·
> smoke 15/0 · dispatch 29/0 · 默认出口 **4/4** · 一键套件 **7/7** · ruff 通过。
> **远端**：已删除 `exp/seam-frontier` 分支（**0 独有提交、完全并入 main**，删前已核）——
> 远端现在只剩 `main` 与 `release-v0.5.0`。

> **（同日补 11）被 CI 打回一次：我把「节点数恒为 8」写进了判据，CI 证明是错的** ——
> 承「补 9」，如实记录一次**判断错误**与它暴露的方法论问题。
>
> **发生了什么**：补 9 那轮我判定「节点数恒为 8、CI 与本机一致」，据此把 `2.1` / `2.2` / `7.1`
> 从**条件判据**改成**硬编码 8**，还把四处文档的 CI 数字一并"更正"成 8 节点 / 112 input。
> **推送后 CI 立刻红**（run 36116073154）：
> `实跑：test_v3_schema 通过 59（失败 3）／7 节点 · 99 input` —— **CI 确实是 7 节点**。
>
> **我的探针错在哪**（关键）：我只把**上游节点从宿主注册表里摘掉**，就宣称"复现了 CI"。
> 那复现的是「**装了宿主、没装上游包**」—— 这种条件下 `import nodes` 仍然成功，
> `_upscaler_cls()` 抛的是 **RuntimeError**，被 `_upscaler_module()` 捕获 ⇒ 退回
> `get_filename_list` ⇒ **仍是 8 节点**。而 CI 的条件是「**宿主 `nodes` 根本导不进来**」。
> ⇒ **探针只复现了"我以为的失败条件"，没复现"真实的失败条件"**；结论因此反了。
>
> **正确复现法**（已留档在本地维护规范文档里）：`sys.modules["nodes"] = None` 后跑该测试
> ⇒ `62/0 · 7 节点 · 99 input`，与 CI 日志**一字不差**。
>
> **改法**：`2.1` / `2.2` 恢复条件判据；`[7]` 节改成与环境无关的自洽断言；
> 文档恢复"两个数并排写"；**H3h 的语义改成「当前环境的真值 ∈ 声明集合」+ 每种数最多 2 种取值**。
> 顺带：H3e（开源卫生）**又抓到我一次**——我把本机探针路径写进了 `tests/test_v3_schema.py`
> ⇒ 已改为"路径见本地维护规范文档，公开仓库不放本机路径"。
>
> **方法论（三条，已进 skill）**
> 1. **构造反例时，要复现"真实的失败条件"，不是"我以为的失败条件"** ——
>    反例没复现出问题，先怀疑反例本身，而不是宣布"问题不存在"。
> 2. **本地绿 ≠ CI 绿**：本批是这条纪律的第一次实战兑现（本地 7/7 全绿，CI 一推就红）。
> 3. **"两个数并排写"不一定是文档缺陷** —— 如果确实存在两种合法环境，那就是**正确形态**；
>    该做的是让机检理解"多环境"（真值 ∈ 声明集合），而不是强行把数字统一成一个。
>
> **门槛**：test_relay_core **392/0** · test_v3_schema **67/0** · review_050 **84/0** · smoke 15/0 ·
> dispatch 29/0 · 默认出口 **4/4** · 一键套件 **7/7**。
> **方法论沉淀（三条）**：① **「全套闸门绿」≠ 仓库健康**（闸门只证明**它自己那套判据**）；
> ② 契约性改动要**横向 grep 同类桩**；③ **「结果正确」与「代码正确」要分开验**。
...
> 直接吃普通张量，喂 H3 的 **NestedTensor 打包 latent 当场炸**
> `'NestedTensor' object has no attribute 'dim'`（一采+二采全跑完，到 SR 那步才死）；
> 且它内部分块**写死 32 帧**，用户没法按显存调。本包按纪律**零 patch 第三方包** ⇒ 适配层建在我们这边。
>
> **性质（读上游源码核过，非"看起来像"）**：**零去噪 / 零重采样**，`target_size=(t, H2, W2)`
> ⇒ **时间维一帧不动**（帧网格、TrimAV 的 22 帧裁量、口型与钉住区逐位连续全不受影响）；
> **音频流完全不碰** ⇒ 回包原样带过，「音频不走 SR」是结构性成立。
>
> **块数自选 + 数值合规**（fail-closed，与 E1 同纪律）：`chunks`（1 = 整段一次过）沿时间维切块，
> 硬约束 **每块 ≥ 2·overlap+1 帧**，超限直接报错并给出**最大合法块数**（绝不偷改参数）；
> 拼接数学与上游同口径（两侧 replicate 填充 + 线性渐变权重 + 按累计权重归一）。
> 第 27 组 **15 条**断言锁住：`chunks=1` 与整段放大**逐位相同**、**分块 == 整段**（max|Δ| = 5.96e-08）、
> 时间维不许变、秩不对/拿错尺子一律 raise、上游口径常量（overlap=5 / 默认块长 32）不许漂移。
>
> **模型与出处纪律（开源语境）**：权重目录沿用作者的 `models/latent_upscale_models/`
> —— **优先直接调作者的 `scan_models()`**，装了上游节点就共用同一份文件，本包不另立目录、不复制第二份；
> 下拉提示与"未装上游"的报错都写明作者仓库与 HF 权重地址。本包**未装上游也照常加载**，只有该节点在被使用时报可照做的安装指引。
>
> 计数与文档已同步：版本三处 0.6.8 · `nodes.py` 头「八个节点」· README 节点表/速览 ·
...
> `H3RelayChain` 加 4 个可选格 —— `prompts`（多行，`---` 分块，第 k 块喂第 k 段）、
> `prompt_target`（写进哪个格子，留空自动探测）、`auto_concat`、`concat_name`；
> 节点上多一个按钮 **🧩 拼成一条**。
> 拼接在**包内**实现（不需要外部 ffmpeg）：画面**流拷贝（无损）** + 音频逐段对齐 + 四项断言，
...
- **连跑 ≠ 换词**：0.6.6 之前的「⏩ 连跑」是**反复提交同一张图**、只改段号 ⇒ N 段词完全相同。
  README §3 教的就是"手改 prompt 再点 Queue"，所以多数用户以为连跑能跑剧集，其实跑的是同一句词的 N 次延续。
- **连跑 ≠ 成片**：全仓 `grep concat|assemble` **零匹配**（旧版），拼片要么靠用户自己敲 ffmpeg，
  要么靠我们产线私有的 `h3_assemble.py`（不在本仓）。

### 词分发（`prompts` / `prompt_target`）

- `prompts` 用**多行文本 + 单独一行 `---`** 分块，**第 k 块喂第 k 段**；留空 = 老行为（一个字不动）。
  为什么不是"词目录"：UI 用户的既有习惯是"在节点上改字"，让他去建 `oner_seg1.txt..N.txt`
  是**把 API 习惯强加给 UI**（零文件系统操作 = 学习成本≈0）。
- 块数 < 要跑的段数 ⇒ **不排队**并报错（绝不静默复用上一块 —— 那正是"以为换了词、其实没换"的坑）。
- `prompt_target` 三种写法：留空自动探测 / `683` / `683.h3_data`。自动探测优先级
  `h3_data`（第三方出词节点的 JSON 串）> 官方 `MiniMaxH3*.prompt` > 其它 `prompt`；
  **同优先级出现多个候选就报错**（让人工指定，不猜）。
- `h3_data` 是 JSON 字符串 ⇒ 前端**解析 → 只改 `prompt` 字段 → 重序列化**；
  解析失败/不是对象 ⇒ **拒写**并把原值留好（绝不覆盖用户那一坨）。
- 纯函数在 `web/relay_kit_prompt.js`（前端与离线单测共用同一份）⇒ 25 条 node 单测锁住边界。

### 拼接成片（包内实现，不依赖外部 ffmpeg）

- 入口两个：`auto_concat` 开着 ⇒ 连跑结束**自动拼**；节点上的 **🧩 拼成一条** 手动拼。
- **段清单怎么来**（不猜文件名）：前端把每段 `queuePrompt` 回来的 `prompt_id` 交上来，
  后端用 ComfyUI 自己的 history 取回那一段的落盘回显 ⇒ 精确到"我刚才跑的那几段"。
  没给 id 时回扫 history 找「图里带本 Chain 节点」的提交（取最近 N 次）；再找不到就**明确报「我找了什么」**，
  不拼半条。
- **画面 = 流拷贝（无损）**：逐包重定位时间戳（pts/dts 同加同一偏移）；实测成片首帧与源段**逐位相同**。
- **音频 = 解码后按段对齐再重编码**（必须）：段文件容器里的 AAC 流比视频**长**（编码器 priming，
  实测 +0.0320 s @32k = 1024 样本；裁过的段还多 ~629 样本尾部填充）。
  逐段「去 priming（`AUDIO_ENCODER_PRIME_MS`，同 AudioSeam 的 `join_prime_ms`）→ 截到本段有效时长
  （= 帧数/fps，同 `join_audio_segments` 的 `segment_seconds`）」，于是**零累积漂移**：
  实测 4 段成片逐段落点偏差 **0 样本**（第 1 段 1 ms）、NCC 0.999。
- **四项断言**（与产线组装器同口径）：帧数守恒 / PTS 无洞 / DTS 递增 / A·VΔ ≤ 1 帧。
  **断言不过 ⇒ 片留着取证，但报告写明"别当成品用"**（不悄悄删、也不假装成功）。
- **退路**：`video="auto"`（默认）= 先走流拷贝，成片断言不过才退回**单遍重编码**重拼。
- **逐段体检先行**（拼之前就拦住）：没有视频流 / 各段分辨率·fps·采样率·声道不一致 /
  **音频比视频长超过 1 帧**（= 该段音频很可能**绕过了「裁重叠」**，每段差 ~0.9s 且逐段累积）。
  实测历史坏样本 `earE5_prod_s2`（A−V = +0.9487 s）被拦下并给出可照做的处置。

### 音频代际 2 → 1（PCM 边车 + 音轨档，2026-09-22 二轮）

**问题**：段文件的音轨是用户落盘时编的 AAC（**第 1 代**）。拼接若「解 mp4 音频 → 重编 AAC」= **第 2 代**。

**做法**：`H3RelayTrimAV`（必备节点）多一个 `save_pcm`（**默认开**）——它手里那份音频
**既与画面等长、又还没经过任何有损编码**（音视频同裁的产物），是整条链上唯一合适的无损源。
落成 `<output>/relay_kit/pcm/seg_<n>_.safetensors`（≈2 MB/段，宿主同款计数器命名 ⇒ 不撞车）。
拼成片时**直读边车**：不解 AAC、**也不需要摸 priming**（边车本身无编码器延迟）。

- **取回不猜文件名**：节点把边车路径**回显进 ui**（键 `h3relay_pcm`）⇒ 宿主收进 `history.outputs[node_id]`
  ⇒ 后端按段取回。老提交/关掉开关的图取不到 ⇒ 当没有。
- **逐段降级，逐段如实标注**：`audio_alignment[].source` = `pcm` / `aac`；缺边车、采样率/声道不符、
  长度不足（补静音）都走各自的处置并写进报告 —— **不 raise、不静默**。
- **成片音轨档 `audio_out`**：`aac_256k`（**默认**，用户口径）/ `aac_192k` / `pcm_lossless`。
  无损档 = mp4 + **PCM f32**（本机 PyAV 实测可写；`.mkv` 亦可）。**不引入 MKV 依赖**：
  宿主 SaveVideo 虽支持 `format=mkv`，但它的音轨恒为 aac/opus ⇒ 从宿主拿不到无损音轨，MKV 没有收益。
- **`video_crf`**（默认 16，0~51）：只在画面**必须重编码**时生效（规格不一致 / 流拷贝断言不过）。
  同时把「拷贝路抛异常」也纳入退路（原先只处理"断言不过"）。
- 🔴 **音频链上还有别的音频节点怎么办**（产线接线 = `裁重叠 → 音频缝 → 落盘`）：真进 mp4 的是
  **链上最后那个**的输出 ⇒「**音频缝也落边车**」（就是它送进落盘的那份音频），
  拼接在多个候选里按「**与 mp4 音频长度最接近**」挑；差 > 100 ms 的候选**拒收**并退回解码
  （容差按容器事实定：priming+尾填充 ≈ 33–53 ms，J-cut/裁量差 ~0.9 s ⇒ 干净分开）。
  判同源靠**长度**，不靠接线假设。

**真实段文件实测**（`ot11_s1..s4`，672×1184 / 702 帧 / 29.25 s，零 GPU）：

| 档位 | 成片音轨 vs 无损源 | 落点 | 体积 | 耗时 |
|---|---|---|---|---|
| `pcm_lossless` + 边车 | **逐位一致（maxΔ = 0）** | 4 段全 **0 样本** | 29.1 MB | **0.17 s** |
| `aac_256k` + 边车 | 40.5 / 40.5 / 50.0 / 47.2 dB | 4 段全 **0 样本** | 22.4 MB | 2.1 s |
| `aac_192k` + 边车 | 40.1 / 40.5 / 44.6 / 38.4 dB | 4 段全 **0 样本** | 22.3 MB | 1.3 s |
| `aac_256k` 无边车（老路） | —（解 AAC 再编） | 段1 **0**；段2~4 相对边车路 **+629 样本（19.7 ms）** | 22.4 MB | 2.2 s |

> 最后一行不是"新 bug"：那是**旧路的 priming 模型残差** —— 段文件容器音频 = 视频 + 1024（priming）
> + ~661（编码器尾填充），旧路按 `min(AUDIO_ENCODER_PRIME_MS, 多出来的量)` 去头 ⇒ 去不干净。
> **边车路结构上没有这个问题**（不经容器、不猜 priming）⇒ 这也是它除了"少一代"之外的第二个收益。
> ⚠ 判这一行时别用"用 raw−want 合成边车"当参照：那会把尾部填充也当 priming 去掉，
> 人为造出 661 样本的假偏移（本轮踩过）。

### 开源可用性：别人（UI **和** API）也走得通（2026-09-22 三轮）

**问题**：0.6.7 的新能力有一半是**前端 JS 实现**的（画布按钮）。用 `/prompt` 提交 JSON 的脚本、
或没有画布的批处理环境，可能"看起来有这个格、其实什么都没发生"。

- **README 新增 §7.4「API / 脚本用户」**：一张表讲清**哪些是 UI 特性**
  （`status` / `prompts` / `prompt_target` / `auto_concat` / `concat_name` / `audio_out` / `video_crf`）；
  脚本提交时这几格**会被忽略（不报错）**，不要靠它们。
- **新增命令行入口 `tools/concat_segments.py`**（第七个脚本）：给不开画布的用户，
  与画布 🧩 按钮**共用同一份核心代码**（不存在第二套拼接实现）：
  ```bash
  python tools/concat_segments.py s1.mp4 s2.mp4 -o film.mp4 [--audio aac256|aac192|lossless] \
         [--crf 16] [--pcm p1.st - ...] [--json]
  ```
  退出码 **0 = 过四项断言**、1 = 不过（脚本可据此判失败）。坏输入给**一句话**，不吐 PyAV 堆栈。
- **宿主版本差异不再静默失效**：`app.queuePrompt` / `api.fetchApi` 缺失 ⇒ **状态格里明说**
  （并指向 CLI/库调用路）；`add_stream_from_template`（av>=17）缺失 ⇒ **自动退回重编码**并在报告写明原因
...
> 再加一道**假阳性**闸（零 GPU 预检抓到）：第三方节点的 `detail_info` 状态文本
> （`📂 文件名称 : xxx.mp4\n📏 物理尺寸 : …`）末尾也是 `.mp4`，会被当成"文件记录"，
> 而且**可能排在真记录前面** ⇒ 拿错路径白跑一轮。判据 = 路径里不会有换行、且除盘符冒号外
> 不含文件名字符集之外的符号（`26.48` 锁）。
...
README §7.1 旧版把 `-c copy`（无损重封装）列为**首选**。0.6.7 在真实 4 段上实测（2026-09-22）：

| 拼法 | 帧数 | 视频时长 | 音频逐段落点（NCC） |
|---|---|---|---|
| `ffmpeg -f concat -c copy` | 702（对数） | **29.346 s**（应 29.250 ⇒ 虚增 96 ms） | 段1 完美 / 段2 +64 ms（0.87）/ 段3 −25 ms（0.21）/ 段4 −33 ms（0.29） |
| 包内拼接（0.6.7） | 702 | 29.250 s | 逐段 **0 样本**（NCC 0.999） |

⇒ README §7.1 已改：**首选包内一键**；要走外部工具就直接上 concat filter 重编码（`-c copy` 不再推荐），
且拼完**必须自检**（帧数守恒 + A/V 时长差 ≤ 1 帧）。

### 铁律一·例外（写在 CONTRIBUTING）

拼接**不做成节点**，而是「包内 route + 画布按钮」：它的输入是**别的节点写出来的 mp4 文件**、
不是张量 —— 节点的连线口拿不到"别人刚往哪个文件写了什么"，只有发起排队的前端知道（prompt_id）。
路由只是入口，核心拼接代码在 `relay_core` 里、与节点共用；结果写回节点 `status` 格。

### 别的

- `requirements.txt`：补 `av` / `numpy` 的**延迟 import** 说明（宿主 ComfyUI 已自带，不用拼接功能不受影响）。
- `tools/check_ui_workflow.py`：新增 **Chain 词分发/拼接**机检（块数 < 段数、`prompts` 乱填分隔、
  `auto_concat` 但图里没有落盘视频节点）。
- `tests/test_prompt_dispatch.mjs`（新）：词分发与段记录纯函数 29 条 node 单测。

## 0.6.6 — 2026-09-21

> **实验层收口（无行为变更）**：E3 删掉冗余的反向 DTW；E2 语义更正（它从来不是 SDEdit）；
> E5 耳测归档。**默认全关 ⇒ 主干逐位不变**（E2 的 σ=0 仍返回同一张量；E3 只是少算一次 DTW）。
> 门槛：`test_relay_core` **304/0** · `test_experimental` **57/0** · `review_050` **80/0** · `smoke_nodes` **15/0**。

### E3 · 删冗余反向 DTW（`head_repeat_dtw`）

- **根因是数学必然，不是 bug**：`dtw_residual` 的局部代价是 `|a−b|`，对调换两个参数**不变**（对称）
  ⇒ DP 总代价矩阵转置不变 ⇒ 最优总代价对称；回溯 tie-break 在镜像输入下也镜像
  ⇒ **两方向路径步数相等、平均代价恒等**。真实链三次实测两列逐位恒等（0.10644 / 0.08962 / 0.17440）。
- 旧 docstring 声称反向代价能「看出是否只是长度差造成的假象」——**该用途永不生效**：
  长度差假象早已由 `dtw_residual` 的「按路径长度归一」消掉（见其 2026-09-20 的 `stay_b` 语义更正）。
- 改动：删第二次 `dtw_residual(fb, fa)`（DTW 是 O(n²)，白算一半）；返回值去掉 `reverse_cost` 键；
  `nodes.py` 的 report 行只打 `align_cost`；对称性改为**性质锁**进单测，不再逐档断言。
- 单测：E3.1–3 由「逐档断言两方向一致」改为「一次显式**对称性质锁**」，并新增 **E3.12b**
  （守 `head_repeat_dtw` 返回值不含 `reverse_cost`）。
- **真跑验证（2026-09-21 22:03）**：复用 canon `onerSpec` 的 s1 latent、只渲 stage 1（2 段 / 672×1184 / cond 桥 /
  `LENGTH=96`），链跑通 —— 段帧数 `[107, 85]`、成片 **192/192 帧守恒、PTS 无洞、DTS 递增、A/VΔ=0.0s**、
  `accept` 退出码 0。TrimAV 报告行按新格式打出：
  `🧪 DTW 对齐代价（只读观测，不改裁量）：窗→钉住区 0.10828｜比较 36 vs 22 帧` —— **不再有「反向」字段**，无格式错误。
  ⚠️ 取证位置：该报告是**节点 `print`**（`nodes.py:611`）⇒ 只进 **ComfyUI 服务端 stdout**
  （`I:/logs/comfyui_<HHMMSS>_out.log`），**不进** `chain_auto.sh` 的链日志、也不进 `/history`。

### E2 · 语义更正：它**不是** SDEdit（**行为不变**）

- 核实比对论文（SDEdit, Meng et al. ICLR 2022, arXiv:2108.01073）三要素 ——
  ① 扰动对象是**去噪初值**（模型会去噪还原它）② 幅度按**调度表 σ_t0** ③ 从 t0 跑**完整反向**
  —— `exp_cond_noise` **一条都不满足**：它给的是 **conditioning 钉帧**加 σ·std 噪声。
- ⇒ 本项实为「**conditioning 参考脏化**」，这也正是它与**硬锁互斥**的真因
  （零 GPU 已量化：σ=0.1/0.25/0.5 ⇒ 钉住块相对扰动 0.100/0.250/0.501，而硬锁自检阈值是 0.00000
  ⇒ 开 E2 = 主动放弃该自检）。
- 只改文案与语义标注：`relay_core.py` 常量区注释 + `sdeedit_noise` docstring、节点 tooltip、
  运行日志行、单测组标题。**函数名与行为保留**（兼容）。
- **真 SDEdit 软钉已另立项**（两遍法：Pass1 采样终态 latent → Pass2 `SetLatentNoiseMask`
  + sigmas 切片从 t0 起），见 `_handover\SDEdit-立项-真软钉-20260921.md`。

### E5 · `exp_bed_jitter` 耳测归档（README §7.3）
- 稳态床源下 **j0 无靶**（三缝听不出重复）、**j12 有伤**（缝后 ~0.7s 电平坑）⇒ 归档，默认保持 0。
- ⚠️ **不是删代码**：默认档不变量继续由单测守；**若将来开 BGM / 用有内容床源，需重开本项**
  （届时下限 `jitter ≥ patch`、上限 `(床长−补丁)/(段数−1)`）。
- 附带更正：22s 处的「音乐」= **生成侧 take 幻觉**（prod 臂也有、且在补丁区外）—— **永不归 E5**。

### 🔴 E2 线关闭（2026-09-22 补记，GG 拍板「E2 定死」）

- **`exp_cond_noise`（本旋钮）**：**无效且与硬锁互斥** ⇒ 永久默认 0。tooltip 与常量区注释已改为
  「🔴 关项归档，勿开」。
- **真 SDEdit（两遍法）**：在包外实现（`l1_api.py --sdedit-*` / `chain_auto.sh` 的 `SDEDIT_*`）
  并实测**对靶无效** ⇒ **整条 E2 线关闭**。
- 结项证据（`_handover\SDEdit-立项-真软钉-20260921.md` §4）：机制五项全达标（**段体 Δ=0.000000 逐位**）；
  三项判据全无改善（色档阶跃 0.0026→0.0028、缝后运动量 ×0.98–1.01、音频缝凹陷 0.0386→0.0392）；
  **扰动 0.039–0.067 vs 靶信号 0.0026–0.0028 ⇒ 大 15–25 倍，负结果不是「剂量不够」**。
- 保留 `sdeedit_noise()` 与 `exp_cond_noise` 入参（兼容），**行为不变**。

## 0.6.5 — 2026-09-21

> **🛡 patch 台词守卫**：`patch_seconds>0` 不再可能吞掉**本段自己**的台词。
> GG 耳检实锤（「这家店撑不过这个月」在成片里只剩「撑不过这个月」）+ 词级时间戳钉责：
> 「这/家/店」0.60–1.70s 被 patch=2.0 整句替换。门槛：`test_relay_core` **304/0**（+第 25 组 4 条）。

### 机制

- patch 应用前，先在本段**原始音频头部 [0, patch)** 探测台词起点（首个人声起振）；
- 找到 ⇒ patch 自动收缩到 `onset − 0.25s`（report 显式打印收缩前后值）；
- 台词太靠前（可用窗 < 0.10s）⇒ **patch 自动关闭**，本段头部原样保留；
- 头部本来就是环境声 ⇒ 行为与关闭时**逐位一致**（零副作用，25.2 锁定）；
- 判据 **宁枉勿纵**：**2×全源中位** + 持续帧滤波 —— 误报代价 = patch 变短，漏报代价 = 吞字；
  🔴 首版用 P10 基线被压力矩阵打回（S2-2/S2-5）：连续 BGM 上 P10=BGM 谷底 ⇒ BGM 乐句峰
  被当台词 ⇒ 守卫误触发、「零副作用」承诺在 BGM 素材上被打破。改回中位后：BGM 峰（±3dB）
  < 2×中位不触发，台词（+14dB）稳触发；S2-2 onset 0.60s 精确、S2-5 逐位一致恢复。
- margin **0.40s**（首版 0.25 被真实素材打回）：能量判据对弱起音滞后实测 0.30s（探测 0.90
  vs ASR 0.60）⇒ 0.25 盖不住；0.40 后真实素材自 ASR 起点 0.60s 起**逐位无损**；
  压力矩阵另证实：BGM+台词床源的台词 +14dB 被床窗判据稳抓（中位失效域只有语音占 >50% 帧）；
- `patch_guard`（节点 widget，默认开）= 关闭开关；`l1_api --patch-guard 0/1`、
  `chain_auto.sh PATCH_GUARD` 同步透传（UI/API 同一套，铁律一）。

### 边界（如实）

- 守卫只保**台词**；缝处保护相应变弱 ⇒ **出词侧段首留白（docs/06）仍是根治**；
- 能量判据对渐强起音天然滞后（实测低估 0.85 vs ASR 0.60）⇒ margin 0.25s 兜底，
  起音再弱仍可能漏 ⇒ 有 `patch_guard` 可关，最终判定是人耳。

## 0.6.4 — 2026-09-21

> **根修版**：床窗语音规避从「单分支补丁」改为「**全路径统一选窗**」—— 瓦片档（`tile>0`，
> 恰是产线脚本默认档）此前**完全没被保护**。附**铁律三**（给建议的规范）。
> 门槛：`test_relay_core` **300/0**（+第 24 组 14 条）· `test_experimental` 57/0 ·
> `review_050` **80/0**（+L11/L12）· `smoke_nodes` 15/0。

### 🔴 修：`tile>0` 的床窗**完全绕过**语音规避（GG：发现问题就从根本处理，不许绕）

**现象**（真人耳检阳性 + 实测）：规避原先只加在 `build_bed` 的 `tile_samples <= 0` 分支，
而 `tile>0` 恰恰是 `chain_auto.sh` 的**默认档**（`TILE_W=1.2`）⇒ 默认档走**没被保护**的那条路。
床源 = 段1（台词覆盖 3.0–4.0s）实测：瓦片 `select=tail` 取 [3.250,4.450] **voiced 62.5%**，
且瓦片被自叠化环铺 `k=2` 遍 ⇒ **上一段台词在补丁区出现 2 遍**（就是用户听到的「对白重叠」）。

**根修**（不是补丁）：
- 所有取床源路径统一走新 `pick_bed_window`（返回 起点/是否换窗/首选窗占比/最终窗占比）；
- 瓦片档阈值 `AUDIO_SEAM_BED_TILE_VOICED_FRAC = 0`（环铺放大 ⇒ **任何**有声帧都不许）；
- E5（`exp_bed_jitter`）在瓦片档**不再被静默忽略**：错开起点映射成合法候选序号，
  各段窗仍互不相同、且都避开语音（旧代码 `tile>0` 时直接忽略 jitter）；
- 全源皆撞 ⇒ tail 档退回**最静窗**（语音残留最少）+ report ⚠，**不 raise**
  （「素材本身以语音为底噪」是合法素材；`TILE_W` 默认非 0 ⇒ raise 会变默认档硬回归）；
- 报告不静默：显式给出「🎙 取样窗内有声帧占比 X%（判据阈值 Y%）」与换窗前后位置；
- 性能：候选遍历先算一次帧 RMS（O(T)），原每候选重算整段为 O(T²)（60s 床源会卡主线程）；
- 同族 rank 约定统一：`quietest_window` 此前只收 2D，3D 输入会让「检查能过、退路崩」；
- `nodes.py` E5 tooltip 删掉「仅在 tile_seconds=0 时生效」的旧限制（不再真实）。

**默认关不变量**：尾部窗不撞语音时起点与旧行为逐位相同、床声逐位相同（24.8 锁定）。

### 🔴 铁律三：「给建议 / 给方案」必须把好与坏都写透，不许只说一半

任何「要不要做 X」的建议必须**同时**给出四块 —— 缺一块就不算建议、不许给结论：

| 必须写 | 内容 |
|---|---|
| **收益** | 做它能得到什么；**谁受益：画布手动用户 vs API 提交者** |
| **代价** | 要改哪些文件 / 会破哪些既有断言或行为 / 连带要同步哪些计数与文档 |
| **退路** | 出错时退到哪一步（`raise` 还是「退回旧行为 + 告警」），两条路各自的后果 |
| **能力边界** | 它**做不到**什么。不写边界的收益描述 = **虚假安全感** |

**反例（真实发生）**：被问「要不要堵 `tile>0` 缺口」时先只说了半句 ——
「`quietest_window` 天然避开高能量区，风险低于 tail 窗那次」，听上去像"不急"；
两分钟后量出：默认档是 `select=tail`、**根本不走 quietest**，瓦片 voiced **62.5%**、
还被 loop **重复 2 遍** —— 结论完全反过来。**半句结论比不给结论更危险。**
⇒ 判定法：**「只说收益」或「只报风险」的回复，一律视为未完成，必须重写。**

### 彻底核实轮（同日追加：审计 + P0 强化 + 判据级 A/B + 真渲染验收）

- **判据级变异（Tier A）**：把 0.6.3 旧代码与新代码放**同一床源、同参数**下对跑 ——
  旧窗 @3.250s（台词区、占比 62%、补丁 RMS **+25.9 dB** 相对底噪 = 台词级），
  新窗 @0.800s（占比 0%、**−7.9 dB** = 纯底噪）。唯一变量 = 代码版本，旧病/新治同尺可量。
- **真渲染验收（Tier B）**：`okBed` 臂（复用 canon s1 latent，`patch=2.0 + tile=1.2` =
  旧代码盲区档）：节点日志选窗 **@0.80s**（与零 GPU 预测逐位一致）；同编码域实测
  补丁区相对台词 **−42.2 dB**（旧代码预期 ≈ −4 dB）；段级 A−V **+0.0237s** 保持。
- **P0 持续帧滤波**：`AUDIO_SEAM_BED_VOICED_SUSTAIN = 2` —— 连续 ≥2 帧（100ms）超阈值
  才计「有声帧」。实测连续台词占比**无损**（58%→58%），单帧瞬态 **0%**（原会触发
  瓦片档换窗）。能量判据的既有边界（低电平/BGM 型漏检）不变，README §7.2 如实声明。
- **性能坑同类复发修补**：`pick_bed_window` 候选能量选窗原逐候选切窗重算 O(n)
  ⇒ 全静音素材上退化为 O(T²/hop)（与 `_frame_rms` 那次同类）；改前缀和 O(候选数)，
  **24.14 锁死「前缀和 == 逐候选参照」**不许改结果。
- **报告措辞诚实化**：已换窗后首行不再自称「尾部窗（与缝天然连续）」（实测曾打出
  「尾部窗…@0.80s」自相矛盾），改「非语音窗（首选尾窗撞语音，已避开）」。
- **退化输入不炸**：prefer 越界钳制 / 全零床源 / 超短床源（<2 帧 ⇒ 放行不判定）。

### 判据能力边界（随规则声明，避免虚假安全感）

能量型判据（帧 RMS 超**全源中位 3×** 记为有声帧）⇒ 只能抓「**安静底噪里冒出来的语音**」
（雨声 + 台词正是这种）；**素材本身以语音为底噪时不报**（BGM / 连续旁白抬高全源中位 ⇒
语音帧不达 3×）⇒ **过了这条检查 ≠「对白重叠」绝迹。**
